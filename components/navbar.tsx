...
export default async function Navbar() {
    ...
    return (
        <header className="h-[64px] w-full border-b border-border bg-bg/95 backdrop-blur-sm flex items-center justify-between px-6 sticky top-0 z-50">
            <div className="flex items-center gap-6"> 
                <Link href="/" className="flex items-center gap-2.5 group">
                    ...
                    <span className="text-lg tracking-tight font-semibold text-fg">MinoManager</span>
                </Link>

                <nav className="flex items-center gap-10 pl-3">
                    {navItems.map((link) => (
                        <Link key={link.href} href={link.href} className="relative py-2 text-base font-medium text-fg-muted transition-colors hover:text-fg group">
                            {link.label}
